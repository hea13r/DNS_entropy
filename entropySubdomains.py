...
# function to calculate the entropy of the subdomain
def calculate_entropy(domain):
    p, lns = Counter(domain), float(len(domain))
    return -sum(count/lns * math.log2(count/lns) for count in p.values())

# function to read a file and parse the subdomains, removing domains and returning entropy scores
def process_domains_file(filename):
    ...
    for subDomain in domains:
        subDomains.append('.'.join(urlparse(subDomain).path.split('.')[:1]))
...
def main():
    ...
    for subDomain, entropy in sortedDict:
        if entropy > 3:
            print(f"Entropy for '{subDomain}': {entropy:.2f}")
